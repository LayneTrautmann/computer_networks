#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────────
# configure-wan-c2c3.sh – Apply tc-netem WAN emulation on C2→C3 path (vm2)
#
# Usage:
#   ./configure-wan-c2c3.sh <scenario>
#
# Scenarios:
...
CONTAINERS=(
    "clab-grocery-wan-c2c3-wan-router"
)
IFACE="eth1"
...
case "$SCENARIO" in
    none)    clear_netem ;;
    low)     apply_netem 10 1 0.1 ;;
    medium)  apply_netem 50 5 0.5 ;;
    high)    apply_netem 100 10 1 ;;
    extreme) apply_netem 200 25 2 ;;
    *)
        echo "Unknown scenario: $SCENARIO"
        echo "Usage: $0 {none|low|medium|high|extreme}"
        exit 1
        ;;
esac
